...
Scoring is deliberately hierarchical:
  lap points -> stint score -> race score -> overall walk-forward score
Each stint is weighted equally within a race, and each target race is weighted
equally in the final aggregate.
...
DEFAULT_LOW_CONFIDENCE_TRAINING_RACES = 5
...
@dataclass(frozen=True)
class Score:
    """Race-level score plus the per-stint scores used to build it."""
...
def _field_relative_rows(
    ...
    for row in rows:
        if row.race_id in eligible_races:
            by_race_lap[(row.race_id, row.lap_number)].append(
                (row.driver_id, row.lap_time)
            )
    ...
    for stint_rows in grouped.values():
        ...
        if (
            not stint_rows
            or stint_rows[-1].lap_number - stint_rows[0].lap_number
            < min_stint_age_span
        ):
            continue
        for row in stint_rows:
            ...
            others = [
                lap_time
                for driver_id, lap_time in by_race_lap[
                    (row.race_id, row.lap_number)
                ]
                if driver_id != row.driver_id
            ]
            if len(others) < 3:
                continue
            ...
            result.append(
                (
                    row.race_id,
                    float(age),
                    float(row.lap_time - median(others)),
                    row.compound,
                    row.driver_id,
                    row.stint_key,
                    row.era,
...
def _raw_stint_slopes(
    ...
    for stint_rows in grouped.values():
        stint_rows.sort(key=lambda r: r.lap_number)
        if (
            len(stint_rows) < DEFAULT_MIN_STINT_LAPS
            or stint_rows[-1].lap_number - stint_rows[0].lap_number
            < min_stint_age_span
        ):
            continue
        valid = [
            r
            for r in stint_rows
            if r.lap_number not in (r.start_lap, r.end_lap)
            and r.lap_number > r.start_lap
        ]
        if len(valid) < 3:
            continue
        baseline = sum(r.lap_time for r in stint_rows[:2]) / 2.0
        fit = _ols(
            [float(r.lap_number - r.start_lap) for r in valid],
            [float(r.lap_time - baseline) for r in valid],
        )
        if fit:
            slopes.append(fit[0])
    return slopes
...
def fit_models(
    ...
    relative = [
        r
        for r in _field_relative_rows(
            training_rows, eligible_races, min_stint_age_span
        )
        if r[3] == compound
    ]
    ...
    raw_slopes = _raw_stint_slopes(
        training_rows, eligible_races, compound, min_stint_age_span
    )
    ...
    return (
        ModelFit(
            ...
            v2_ci[0],
            v2_ci[1],
            len({r[0] for r in relative}),
            len({r[5] for r in relative}),
            len(relative),
        ),
        ModelFit(
            raw_slope,
            ...
            sum(
                ...
                if r.race_id in eligible_races and r.compound == compound
            ),
        ),
    )
...
def score_target(
    ...
    """Score one compound using equal-weight per-stint metrics.

    Each stint gets its own correlation/RMSE over its valid laps. The returned
    race-level metric is the arithmetic mean of those stint metrics, so long
    stints cannot dominate a race simply because they contain more lap points.
    """
    rel = [
        r
        for r in _target_relative_rows(target_rows, min_stint_age_span)
        if r[3] == compound
    ]
    ...
    if not stint_scores:
        return Score(None, None, 0, 0, tuple())

    valid_corrs = [s.correlation for s in stint_scores if s.correlation is not None]
    valid_rmses = [s.rmse for s in stint_scores if s.rmse is not None]
    return Score(
        correlation=sum(valid_corrs) / len(valid_corrs) if valid_corrs else None,
        rmse=sum(valid_rmses) / len(valid_rmses) if valid_rmses else None,
        n_stints=len(stint_scores),
        n_points=sum(s.n_points for s in stint_scores),
        stint_scores=tuple(stint_scores),
    )
...
def load_rows(db, start_year: int, end_year: int) -> list[LapRow]:
    ...
    return [
        LapRow(
            ...
            str(x["era"]),
            int(x["race_entry_id"]),
            f"{int(x['race_id'])}:{int(x['race_entry_id'])}:{int(x['stint_number'])}",
...
def _low_confidence(compound_usable_training_races: int) -> bool:
    return compound_usable_training_races < DEFAULT_LOW_CONFIDENCE_TRAINING_RACES
...
def run(db, start_year: int, end_year: int, min_training_races: int):
    ...
    for target in metas:
        ...
        train_ids = {
            m.race_id
            for m in training
            if diversity.get(m.race_id, 0) >= DEFAULT_MIN_UNIQUE_PIT_LAPS
        }
        if not train_ids:
            continue
        ...
        for compound in COMPOUNDS:
            ...
            stability.append(
                {
                    ...
                    "era": target.era,
                    "compound": compound,
                    "target_race_index": stable_index[target.era],
                    "slope": v2.slope,
                    "ci_low": v2.ci_low,
                    "ci_high": v2.ci_high,
            ...
            for model, score in scores.items():
                for stint_score in score.stint_scores:
                    stint_scores_output.append(
                        {
                            ...
                            "compound": compound,
                            "model": model,
                            "stint_key": stint_score.stint_key,
                            "correlation": stint_score.correlation,
                            "rmse": stint_score.rmse,
                            "scored_points": stint_score.n_points,
                        }
                    )
                ...
                summary.append(
                    {
                        ...
                        "slope": raw.slope
                        if model == "raw"
                        else (v2.slope if model == "v2" else 0.0),
                        "ci_low": v2.ci_low if model == "v2" else None,
                        "ci_high": v2.ci_high if model == "v2" else None,
                        "prior_same_era_races": prior_same_era_races,
                        "compound_usable_training_races": compound_usable_training_races,
                        "n_training_stints": v2.n_stints,
                        "n_training_laps": v2.n_laps,
    ...
    return summary, stability, stint_scores_output
...
def _overall_race_scores(summary: list[dict], model: str):
    """Collapse compound rows to one score per target race, then average races."""
    by_target: dict[int, list[dict]] = defaultdict(list)
    for row in summary:
        if row["model"] == model and row["race_score_rmse"] is not None:
            by_target[row["target_race_id"]].append(row)

    race_scores = []
    for target_race_id, rows in by_target.items():
        corrs = [
            r["race_score_correlation"]
            for r in rows
            if r["race_score_correlation"] is not None
        ]
        rmses = [r["race_score_rmse"] for r in rows if r["race_score_rmse"] is not None]
        race_scores.append(
            {
                "target_race_id": target_race_id,
                "race_correlation": sum(corrs) / len(corrs) if corrs else None,
                "race_rmse": sum(rmses) / len(rmses) if rmses else None,
            }
        )

    correlations = [r["race_correlation"] for r in race_scores if r["race_correlation"] is not None]
    rmses = [r["race_rmse"] for r in race_scores if r["race_rmse"] is not None]
    return {
...
def main() -> int:
    ...
    parser.add_argument(
        "--min-training-races",
        type=int,
        default=DEFAULT_MIN_TRAINING_RACES,
    )
    ...
    parser.add_argument(
        "--stint-csv", default="tyre_model_walkforward_v2_stints.csv"
    )
    ...
    try:
        summary, stability, stint_scores = run(
    ...
    print(
        "targets_scored="
        f"{len({r['target_race_id'] for r in summary})}"
    )
    print(
        "Scoring hierarchy: lap points -> equal-weight stints -> equal-weight races"
    )
    for model in ("raw", "v2", "flat"):
        overall = _overall_race_scores(summary, model)
        print(
            f"{model}: overall_race_rmse={overall['overall_rmse']} "
            f"overall_race_correlation={overall['overall_correlation']} "
            f"target_races={overall['target_races']}"
        )
    ...
    for r in sample:
        print(
            f"{r['era']} / {r['compound']}: "
            f"prior_same_era_races={r['prior_same_era_races']} "
            f"compound_usable_training_races={r['compound_usable_training_races']} "
            f"slope={r['slope']} "
    ...
    print(
        f"\nWrote {args.csv}, {args.stability_csv}, and {args.stint_csv}"
    )
