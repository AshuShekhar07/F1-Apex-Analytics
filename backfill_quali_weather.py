...
print(f"{len(sessions)} Q sessions need weather backfill")
    ...
    for i, s in enumerate(sessions):
        ...
        try:
            ...
            if wdata is None or len(wdata) == 0:
                print(f"  [{i+1}/{len(sessions)}] {year} R{rnd} Q: no weather data, skipping")
                continue
            ...
            conn.commit()
            print(f"  [{i+1}/{len(sessions)}] {year} R{rnd} Q: OK (track {track_temp:.1f}C, rain={rainfall})")
        except Exception as e:
            print(f"  [{i+1}/{len(sessions)}] {year} R{rnd} Q: FAILED ({e})")
        time.sleep(0.3)
