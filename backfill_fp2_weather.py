...
with engine.connect() as conn:
    sessions = conn.execute(text("""
        SELECT s.id AS session_id, r.season_year, r.round_number
        FROM sessions s
...
print(f"{len(sessions)} FP2 sessions need weather backfill")
    ...
    for i, s in enumerate(sessions):
        ...
        try:
            ...
            if wdata is None or len(wdata) == 0:
                print(f"  [{i+1}/{len(sessions)}] {year} R{rnd} FP2: no weather data, skipping")
                continue
            ...
            conn.commit()
            print(f"  [{i+1}/{len(sessions)}] {year} R{rnd} FP2: OK (track {track_temp:.1f}C, rain={rainfall})")
        except Exception as e:
            print(f"  [{i+1}/{len(sessions)}] {year} R{rnd} FP2: FAILED ({e})")
        time.sleep(0.3)
